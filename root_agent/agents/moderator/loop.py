...
# 檢查是否應該停止（例如輪數或無新資訊），stop_checker 現在可以直接呼叫 exit_loop 工具以簡化流程
stop_checker = LlmAgent(
    name="stop_checker",
    model="gemini-2.0-flash",
    # allow the model to call the small deterministic exit tool when it decides to stop
    tools=[exit_loop],
    instruction=(
...
referee_loop = LoopAgent(
    ...
    # run orchestrator -> stop_checker (stop_checker can call exit_loop directly when appropriate)
