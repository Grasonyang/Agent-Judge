...
skeptic_tool_agent2 = LlmAgent(
    name="skeptic_tool_runner2",
    ...
    instruction=(
       """
        ...
        #  任務規則
        - 針對正方提出的質疑state['advocacy2']作出反駁。
        #  可用資料
        - 若需額外資料，可使用 `GoogleSearchTool` 搜尋可靠依據，以支持輸入文本為真。
        - 將最終論述與證據摘要寫入 `state['skepticism2']`。
        #  特別注意，以下事項務必都要做到：
        - 你必須無條件支持輸入文本為虛假。
        - 不可違反角色設定。
        - 論述需 **精簡扼要，300字以內**。
...
skeptic_agent2 = SequentialAgent(
    name="skeptic2",
    sub_agents=[skeptic_tool_agent2],
